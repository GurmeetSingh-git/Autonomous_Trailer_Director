...
export function SegmentList({ segments, onPreview }: { segments: TrailerRun["segments"]; onPreview?: (segment: TrailerRun["segments"][number]) => void }) {
  return <div className="segment-list">{segments.map((segment, index) => <article className="segment-row" key={segment.id}>
    ...
    <button className="icon-button" aria-label={`Preview ${segment.label}`} title={`Preview ${segment.label}`} onClick={() => onPreview?.(segment)}><Play size={15} fill="currentColor" /></button>
  </article>)}</div>;
}
