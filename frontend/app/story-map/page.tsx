...
import { demoStoryMap, getStoryMap, retryStoryMap, type StoryMap } from "@/lib/api";
...
function StoryMapPage() {
  ...
  useEffect(() => {
    ...
    getStoryMap(runId)
      ...
      .catch(() => {
        if (!active) return;
        setError("This uploaded run could not be loaded. A saved checkpoint may still be recoverable.");
        setCanRecover(true);
      });
    return () => { active = false; };
  }, [runId]);
  ...
  async function handleRetry() {
    ...
    setError("");
    try {
      await retryStoryMap(runId);
      const recoveredMap = await getStoryMap(runId);
      setMap(recoveredMap);
      setCanRecover(false);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "The saved analysis could not be recovered.");
      setCanRecover(true);
    } finally {
      setRecovering(false);
    }
  ...
  if (error) return <main className="page"><div className="eyebrow">Story map / unavailable</div><h1 style={{ fontSize: 54 }}>Run unavailable.</h1><p className="lede">{error}</p>{canRecover && <button className="primary-button" onClick={handleRetry} disabled={recovering}>{recovering ? "Recovering..." : "Retry saved analysis"}</button>}<Link className="mono" href="/">Return to upload</Link></main>;
